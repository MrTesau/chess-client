...
// instead of copying rules all over the show:
// RulesObject -> RulesObject.pieceName(CurrPos, TargetPoss)

// Another IDea:
// instead of mapping to render sqaures
// map sqaures once, save each component in an array
//render the array
// on select set boardContainer select as normal
// on move trigger move, find target in the arr eg arrofSauares[target] to asses if its ok to move
// if ok change internally state of square component and send chesspiece object to new square
// saves mapping of entire squares/board on each move/render

// autoplay brainstorm
// useeffect on turn = computer
//settimout for experience

// find units it can take
// find units it can lose
// make highest point movie
// if none move closest unit to enemy king
...
// ** the attempt
// square Component Arr
const createArrayOfSquareComponents = (squares) => {
  let components = [];
  squares.map((square, index) =>
    components.push(
      <Square
        ...
        isSquareSelected={square.isSquareSelected}
        occupied={square.occupied}
      />
    )
  );
...
const Square = ({
  ...
  rightDown,
  //handleSelect, // select a square w/unit
  isSquareSelected, // current square is selected
  occupied, // current square is occupied
  select, // after selecting a square with a unit adopts that unit
  setSelect,
  squares,
  setSquares,
  arrOfSquareComponents,
  setArrOfSquareComponents,
  // moveUnit, // function to move unit to a new square
  col, // col 1-8
}) => {
  const handleSelect = (idx) => {
    // set Parent Select state
    setSelect(squares[idx]);
    let newComponents = arrOfSquareComponents;
    newComponents[key] = (
      <Square
        select={select}
    ...
    setArrOfSquareComponents(newComponents);
    // handle like a movement to pass isSelected back in
  };

  const moveUnit = (destinationIdx) => {
    // target square object
    let destinationSquareObj = occupied;
    //  Trying to move to a square with another unit
    //  setSelect(new Square) and stop movement attempt
    if (
      squares.filter(
    ...
    // implement occupied squares rules
      ...
      // turn off occupied
      let newComponents = arrOfSquareComponents;
      newComponents[select.idx - 1] = (
        <Square
          // props passed current state value of select
          // square object
          select={select}
          // props passed to tell if square is 'selected' boolean
          isSquareSelected={false}
          occupied={false}
          // og key 0 indexed, so 1 minus selects idx is correct
          key={select.idx - 1}
          // Diagonal connecting arrays
          leftUp={select.leftUp}
          rightUp={select.rightUp}
          leftDown={select.leftDown}
          rightDown={select.rightDown}
          // Horizontal and Vertical position
          row={select.row}
          col={select.col}
          // Index of square 1-64
      ...
      // pass in square object saved in select
      // retain all other data (only piece is switched not square coordinates)
      newComponents[destinationIdx - 1] = (
        <Square
          // props passed current state value of select
          // square object
          select={select}
          // props passed to tell if square is 'selected' boolean
          isSquareSelected={false}
          occupied={select.occupied}
          // og key 0 indexed, so 1 minus selects idx is correct
          key={select.idx - 1}
          // Diagonal connecting arrays
          leftUp={destinationSquareObj.leftUp}
          rightUp={destinationSquareObj.rightUp}
          leftDown={destinationSquareObj.leftDown}
          rightDown={destinationSquareObj.rightDown}
          // Horizontal and Vertical position
          row={destinationSquareObj.row}
          col={destinationSquareObj.col}
          // Index of square 1-64
          idx={destinationSquareObj.idx}
          //handleSelect={handleSelect}
          //moveUnit={moveUnit}
        />
      );
  ...
  return (
    <div
      ...
      style={{
        minHeight: "12.5%",
        minWidth: "12.5%",
        border: !isSquareSelected ? "1px solid black" : "2px solid red",
        background: setBg(row),
        color: "red",
        overflow: "hidden",
        fontSize: "12px",
...
//////
// uses an array of objects to create an array of squareComponents
const CreateBoard = ({ setCurrentBG, wowBg, gotBg }) => {
  const [renderedPropComponent, setR] = React.useState(false);
  const [squares, setSquares] = React.useState(
    setBattleground(gameOfThrones_1)
  );
  const [arrOfSquareComponents, setArrOfSquareComponents] = React.useState(
    createArrayOfSquareComponents(squares)
  ); // arr of square Components
  const [select, setSelect] = React.useState(undefined); //defined once an occupied square has been clicked, stores square object
  //const [mute, setMute] = React.useState(false);
  // array of square objects
  // function accepts [{square}..etc] and returns [<Square />]
  const createArrayOfSquareComponentsWithProps = (squares) => {
    let components = [];
    squares.map((square, index) =>
      components.push(
        <Square
          ...
          arrOfSquareComponents={arrOfSquareComponents}
          setArrOfSquareComponents={setArrOfSquareComponents}
          setSelect={setSelect}
          key={index}
          // Diagonal connecting arrays
          leftUp={square.leftUp}
    ...
    setR(true);
    setArrOfSquareComponents(components);
  };
  const audioReaction = (selectSquare) => {
    if (!selectSquare /*|| mute*/) return;
    //Can we play via select => audio of selected
  ...
  // first thing is call setArrComponents with createArr(squares)
  return (
    <>
      {!renderedPropComponent ? (
        createArrayOfSquareComponentsWithProps(squares)
      ) : (
        <></>
      )}
      {arrOfSquareComponents}
